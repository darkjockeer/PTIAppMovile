...
class _drawerpomaceas extends State<drawerpomaceas>{
  ...
  Future comprobacion() async {
    try {
      var rest = await http.post(
          Uri.parse(
            EndPoints.generateEndPointsURLInformes('/informe.php'),
          ),
          body: {
            'fecha': fechas[0],
            'turno': valor5,
    ...
    if (statuses[Permission.storage]!.isGranted) {
      ...
      if (dir != null) {
        ...
        try {
          ...
          print(link);
          await Dio().download(
            link, savePath, onReceiveProgress: (received, total) {
            if (total != -1) {
  ...
  Future idinforme() async {
    try {
      ...
      print(url);
      var rest = await http.post(url, body: {
        'fecha': fechas[0],
        'turno': valor5,
        'idcentral': valor6,
        'idespecie': especie.toString(),
  ...
  Future Guardarinfo(int nums) async {
    try {
      var restS = await http.post(
          Uri.parse(
            EndPoints.generateEndPointsURLtabla4('/informepomaceas.php'),
          ),
          body: {
