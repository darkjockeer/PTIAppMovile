...
List <String> calibre3=[];
List <String> calibre4=[];

class graficosegregacion0 extends StatefulWidget{
    const graficosegregacion0 ({Key? key}): super(key: key);
    @override
   // ignore: library_private_types_in_public_api
   _graficosegregacion0  createState() => _graficosegregacion0 ();
  }

class _graficosegregacion0  extends State<graficosegregacion0 >{
  ...
  late int count;
        @override
    void initState(){
      valor6=el.nombreidexportador[0];
...
void _addSeries() {
  ...
  {
    chartData2.add(ChartData(calibre4[i] ,calibre2[i]));
  }
  for(int x=0; x<2; x++)
  {
    print(x);
  ...
  var restssrs = await http.post(
      Uri.parse(EndPoints.generateEndPointsURLtabla3("/segregacion.php")),
      body: {
        'idcentral': id,
        ...
        for (int i=0; i<3;i++)
        {
          ...
          if(i==0)
          {
            for(int x=0; x<resBody2['valorexp'].length;x++){
              if(resBody2['valorexp'][x]!='0')
              {
                calibre3.add(resBody2["nombreexp"][x]);
          ...
          {
            for(int x=0; x<resBody2['nombreexp2'].length;x++){
              if(resBody2['valorexp2'][x]!='0')
              {
                calibre4.add(resBody2["nombreexp2"][x]);
