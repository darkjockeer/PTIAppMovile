...
class Solidos extends StatefulWidget{
    const Solidos({Key? key}): super(key: key);
    @override
   // ignore: library_private_types_in_public_api
   _Solidos createState() => _Solidos();
  }
int i=0;
class _Solidos extends State<Solidos>{
  ...
  void _addSeries() {
    ...
    chartData1.add(ChartData('SS < 16.5' ,calibre1[0], calibre2[0]));
    chartData1.add(ChartData('SS ≥ 16.5 - ≤ 17.5', calibre1[1], calibre2[1]));
    chartData1.add(ChartData('SS > 17.5', calibre1[2], calibre2[2]));
    ...
    series!.add(ColumnSeries<ChartData, String>(
      key: ValueKey<String>('${series!.length}'),
      name: 'Industria',
      animationDuration: 1500, animationDelay: 1500, color: ('#4C4C51'.toColor()), dataLabelSettings: const DataLabelSettings( isVisible: false, showCumulativeValues: true,textStyle: TextStyle(fontSize: 10, fontWeight: FontWeight.bold,))
       ,width:0.9, 
      dataSource: chartData1,
      xValueMapper: (ChartData sales, _) => sales.x,
      yValueMapper: (ChartData sales, _) => sales.y1
  ...
  var restssrs = await http.post(
      Uri.parse(EndPoints.generateEndPointsURLtabla3("/solidos.php")),
      body: {
        'idcentral': id,
